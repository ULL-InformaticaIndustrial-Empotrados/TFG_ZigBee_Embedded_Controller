...
bool CDummy::CreateVirtualSensor(int HwdID, std::string ssensorname, std::string ssensortype, std::string soptions)
{
	bool bCreated = false;
...
namespace http {
	namespace server {
		void CWebServer::RType_CreateVirtualSensor(WebEmSession & session, const request& req, Json::Value &root)
		{
			...
			if (CDummy::CreateVirtualSensor(HwdID, ssensorname, ssensortype, soptions))
			{
				root["status"] = "OK";
